...
- mention that only results for R will be shown here, though it is possible that other correlations can be determined, and that this was used in preliminary studies doing a multiparamter combination with R and phi


All of the samples for the completed jobs were fit as described in the previous sections, the \R values were compared to each other among the different analyses, and the correlations calculated. The correlation matrix between different analyzers for the EG dataset is shown in \figref{fig:corrMatAnalyzer}. Tables for the correlation coefficients and their errors for all four datasets are given in Tables~\ref{tab:Corrs_60h_analyzer} through \ref{tab:Corrs_EG_analyzer}. 
