...
As mentioned previously, events were generated through \texttt{TF2->GetRandom2()} method calls. Many samples of pseudo-data were generated via the same procedure, for each dataset and for the two \texttt{TF2} variants with input from either \RE or \RW. The procedure is as follows:
\begin{itemize}
	\item{test}
\end{itemize}
...
First the number of hits was determined such that statistics could be produced such that the error on \R was comparable to the \Rone datasets. These numbers of hits are calculated by \texttt{PoissonD()} method calls on a static number of entries for each dataset, such that each sample has very slightly different amount of entries. The approximate level of stats for the 60h, HK, 9d, and EG datasets are given by \{5.04e9, 7.01e9, 1.08e10, 2.21e10\}, where the numbers were determined empirically. For each hit generated, a time and an energy were generated according to one of the two \texttt{TF2}s described previously, with the input either from \RE or \RW. 



There was no time randomization ... could be included but would take a lot longer ... mean R value would stay the same with enough seeds though the width would increase a bit - the correlations should be affected practically negliblely 
...
In all cases the randomization is performed via ... event randomization, number of events, ratio randomization, EvW randomization
...
-mention number of seeds for the ratio method... cite my thesis to describe how the ratio is constructed?
